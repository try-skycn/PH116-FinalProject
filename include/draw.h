...
#define draw_h
#include "resources.h"
void draw_circuit(const std::vector <std::complex<double> > &I, const std::vector <conductor>& data){
    std::ofstream draw("out.tex");
    
//    直流电路
    if (TYPE == DC){
        draw << "digraph G{" << std::endl;
        draw << "graph[splines = ortho];" << std::endl;
...
//    交流电路
        ...
        for (int i = 0; i < data.size(); ++i){
            ...
            if (data[i].elect_info.emf != std::complex<double>(0,0) && data[i].elect_info.imp != std::complex<double>(0,0)){
                ...
                draw << "be" << "->" << "n" << data[i].edge_info.to << ";" << std::endl;
                continue;
            }
            //只有emf
            if (data[i].elect_info.emf != std::complex<double>(0,0)){
            ...
            //只有imp
                ...
                draw << "n" << data[i].edge_info.from << "->" << "bz" << i << " [label = \"I" << i << "\"];"<< std::endl;
                draw << "bz" << "->" << "n" << data[i].edge_info.to << ";" << std::endl;
            }
